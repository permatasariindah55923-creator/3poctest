...
func TestDoubleDeductionBugFeeDelegationWithRatio(t *testing.T) {
	...
	// Setup
	addr := common.HexToAddress("0x26769d8aaa148d17d470052b42edff604e4ec94b")
	nonce := uint64(0)
	gasLimit := uint64(50000)
	gasPrice := big.NewInt(10 * params.Gkei) // 10 Gwei

	// Calculate fee ratio split (60% for feePayer, 40% for sender)
	feeRatio := types.FeeRatio(60)
	...
	mockMsg.EXPECT().CheckNonce().Return(true).AnyTimes()
	mockMsg.EXPECT().ValidatedSender().Return(addr).AnyTimes()
	mockMsg.EXPECT().ValidatedFeePayer().Return(addr).AnyTimes()  // SAME ADDRESS - This triggers the bug
	mockMsg.EXPECT().FeeRatio().Return(feeRatio, true).AnyTimes() // Fee delegation with ratio
	mockMsg.EXPECT().Nonce().Return(nonce).AnyTimes()
	mockMsg.EXPECT().Hash().Return(common.Hash{}).AnyTimes()
	mockMsg.EXPECT().Gas().Return(gasLimit).AnyTimes()
	mockMsg.EXPECT().GasPrice().Return(gasPrice).AnyTimes()
	mockMsg.EXPECT().GasFeeCap().Return(big.NewInt(0)).AnyTimes()
	mockMsg.EXPECT().GasTipCap().Return(big.NewInt(0)).AnyTimes()
	mockMsg.EXPECT().BlobHashes().Return(nil).AnyTimes()
	mockMsg.EXPECT().AuthList().Return(nil).AnyTimes()
	mockMsg.EXPECT().Value().Return(value).AnyTimes()
	mockMsg.EXPECT().Data().Return(nil).AnyTimes()
	mockMsg.EXPECT().EffectiveGasPrice(gomock.Any(), gomock.Any()).Return(gasPrice).AnyTimes()

	mockStateDB := mock_vm.NewMockStateDB(ctrl)
	mockStateDB.EXPECT().GetNonce(addr).Return(nonce).AnyTimes()
	mockStateDB.EXPECT().GetBalance(addr).Return(balance).AnyTimes()
	mockStateDB.EXPECT().SubBalance(addr, gomock.Any()).Do(func(addr common.Address, amount *big.Int) {
		// Track balance changes to detect double deduction
		balance = new(big.Int).Sub(balance, amount)
	}).AnyTimes()
	mockStateDB.EXPECT().AddBalance(gomock.Any(), gomock.Any()).Return().AnyTimes()
	mockStateDB.EXPECT().GetCodeSize(gomock.Any()).Return(0).AnyTimes()
	mockStateDB.EXPECT().GetCodeHash(gomock.Any()).Return(common.Hash{}).AnyTimes()
	mockStateDB.EXPECT().Exist(addr).Return(true).AnyTimes()
	mockStateDB.EXPECT().Empty(gomock.Any()).Return(true).AnyTimes()

	config := params.TestChainConfig
	header := &types.Header{Number: big.NewInt(0), Time: big.NewInt(0), BlockScore: big.NewInt(0)}
	blockContext := NewEVMBlockContext(header, nil, &common.Address{})
	txContext := NewEVMTxContext(mockMsg, header, config)
	evm := vm.NewEVM(blockContext, txContext, mockStateDB, config, &vm.Config{})
	...
	if err != nil {
		...
		t.Logf("This is expected if the fix properly rejects double-deduction")
	} else {
		t.Logf("buyGas succeeded with no error")
		t.Logf("Final balance after buyGas: %s", balance.String())

		if balance.Sign() < 0 {
			t.Errorf("BUG DETECTED: Balance went negative! This indicates double deduction.")
			t.Errorf("Initial balance: %s", requiredTotal.String())
			t.Errorf("Final balance: %s", balance.String())
			t.Errorf("Total fees deducted: %s (should be once, not twice)", new(big.Int).Add(senderFee, feePayerFee).String())
		}
	}
}

// TestFeeRatioCalculation verifies that CalcFeeWithRatio works correctly
func TestFeeRatioCalculation(t *testing.T) {
	totalFee := big.NewInt(100)
	feeRatio := types.FeeRatio(60)

	feePayerFee, senderFee := types.CalcFeeWithRatio(feeRatio, totalFee)

	// 60% for feePayer = 60
	assert.Equal(t, big.NewInt(60), feePayerFee)
	// Remainder for sender = 40
	assert.Equal(t, big.NewInt(40), senderFee)
	// Total should equal input
	totalRecalc := new(big.Int).Add(feePayerFee, senderFee)
	assert.Equal(t, totalFee, totalRecalc)
}
